...
function gateKeeper(req, res, next) {
  // your code should replace the line below
    ...
    const headerContent = parseHeaderContent(req);
    const user = headerContent.user || null;
    const pass = headerContent.pass || null;
    findUser(user, pass, req);

  next();
}

function parseHeaderContent(req) {
  return queryString.parse(getHeaderContent(req));
}

function getHeaderContent(req) {
  return req.get('x-username-and-password');
}

function findUser(user, pass, req) {
